...
Resetは実行前に確認を表示します。キャンセルすると現在のゲームを継続します。
...
仮想キーボードは表示確認とマウス／タッチ入力のためのもので、ゲームパッドからは操作されません。
ゲームパッドは仮想キーボードの表示状態に関係なく、常にJR-100のジョイスティックとして動作します。

BEEP音は`Sound on`のときに再生されます。
ブラウザーが許可している場合は、ROMの読み込み後に音声を自動で開始します。自動再生が制限されている場合は、ゲーム画面の上に「画面をマウスでクリックして、音声を有効にしてください」と表示します。画面・案内・`Enable sound`のクリック、画面のタップ、またはキー入力で音声を有効にできます。音声が有効になると案内は消えます。保存したミュート設定は維持します。ゲームパッドだけではブラウザーの音声制限を解除できないことがあります。
